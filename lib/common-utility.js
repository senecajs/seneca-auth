module.exports = function() {
  ...
  function do_respond(msg, next) {
    ...
    req.seneca.act("role: 'auth', cmd: 'redirect'", { kind: action }, function(
      errRedirect,
      redirect
    ) {
      if (err) {
        if (redirect) {
          ...
          return next(null, {
            http$: {
              status: forceStatus || 301,
              redirect: forceRedirect || redirect.fail
            }
          })
        }

        return next(null, {
          http$: { status: forceStatus || 200, redirect: forceRedirect },
          ok: false,
          why: err
        })
      }
      ...
      seneca.act(
        "role:'auth', cmd:'clean'",
        { user: req.seneca.user, login: req.seneca.login },
        function(err, out) {
          if (err) {
          ...
          if (redirect) {
            ...
            out.http$ = {
              status: forceStatus || 301,
              redirect: forceRedirect || redirect.win
            }
          }
