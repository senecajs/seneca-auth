  ...
  function buildservice() {
        ...
        if (service != 'local') {
          if (!opts.service[service].action || opts.service[service].action == 'login') {
            func = function (err, user, info) {
              seneca.act({role: 'auth', trigger: 'service-login', service: service}, user,
                function (err, user) {
                  if (err) return next(err);
                  seneca.act({role: 'user', cmd: 'login', nick: user.nick, auto: true}, seneca.err(next, function (out) {
                    return out.ok ? next() : next(err)
                  }))
                }
              )
            }
    ...
    function redirection(req,res,kind) {
      ...
      if( redirect ) {
        ...
        redirect = {
          win:  prefix + ( (rk && rk.win)  || opts.redirect.win ), 
          fail: prefix + ( (rk && rk.fail) || opts.redirect.fail ),  
        }
      }
