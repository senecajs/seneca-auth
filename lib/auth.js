...
module.exports = function auth(opts,cb){
  ...
  function aliasfields(which,src) {
    var out = _.clone(src)
    _.each(opts[which].fields,function(field,alias){
...
*/
  ...
  useract.entity({kind:'user'},seneca.err(cb,function(user){
    userent = user
  }))
   */
  ...
  function buildservice() {
    ...
    function route_register(req,res,next) {
      var details = aliasfields('register',req.body)

      var redirect = redirection(req,res,'register')

      useract.register(details,function(err,out){
        if( err || !out.ok ) { return respond(err,res,redirect.fail,{ok:false,why:out.why}) }
    ...
    function route_reset_create(req,res,next) {
      var redirect = redirection(req,res,'reset_create')

      var nick  = req.body.nick || req.body.username
    ...
    function route_reset_load(req,res,next) {
      var redirect = redirection(req,res,'reset_load')
    ...
    function route_reset_execute(req,res,next) {
      var redirect = redirection(req,res,'reset_execute')
    ...
    var routes = {}
    routes[opts.urlpath.login]    = route_login
    routes[opts.urlpath.logout]   = route_logout
    routes[opts.urlpath.instance] = route_instance
    routes[opts.urlpath.register] = route_register
    routes[opts.urlpath.reset_create]  = route_reset_create
    routes[opts.urlpath.reset_load]    = route_reset_load
    routes[opts.urlpath.reset_execute] = route_reset_execute
  ...
  seneca.act({role:'web',use:buildservice()})
