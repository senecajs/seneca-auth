...
module.exports = function auth(opts,cb){
  ...
  seneca.add({role:name,cmd:'service'},function(args,done){
    var service_init = require('./'+args.service)
    service_init.call(this,args.conf,userent,useract,passport,done)
  })
  ...
  seneca.util.recurse( 
    ...
    function(service,next){
      ...
      seneca.act({role:name,cmd:'service',service:service,conf:conf},function(err){
        next(err)
      })
    },
    function(err){
    ...
    _.each( opts.service, function(conf,service){
      pp_auth[service] = function(req,res,next){
        ...
        passport.authenticate( service, function(err,user,info){
          ...
          var rurl = (prefix||'') + opts.redirect.login.fail
          if (user) { 
            rurl = (prefix||'') + opts.redirect.login.win
          }
