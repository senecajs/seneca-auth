...
module.exports = function auth( options ) {
  ...
  function cmd_clean( args, done ) {
    ...
    var user  = seneca.util.clean( (args.user && args.user.data$()) || {} )
    var login = seneca.util.clean( (args.login && args.login.data$()) || {} )
      ...
      return function(req,res,next) {
        for( var cI = 0; cI < checks.length; cI++ ) {
          ...
          if( restrict && !(req.seneca && req.seneca.user) ) {
            ...
            if( redirect ) {
              res.redirect( options.redirect.restrict )
            }
            else {
              res.writeHead(401)
