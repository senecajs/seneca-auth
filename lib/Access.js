...
//  seneca.add({ role: plugin, cmd: 'login' },              this._cmdLogin(seneca, options))
  seneca.add({ role: plugin, trigger: 'service-login' },  this._triggerServiceLogin(seneca, options))
...
Access._triggerServiceLogin = function(seneca, options) {
  var userent = seneca.make$('sys/user')
  ...
  return function( args, done ) {
    ...
    userent.load$(q, function(err, user) {
      ...
      if( !user ) {
        if (options.user.autoregister === false) {
          done(null, {ok:false, why:'user-not-found'})
        }
        else {
