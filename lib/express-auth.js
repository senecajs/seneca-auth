...
module.exports = function(options) {
  ...
  function registerService(service, conf) {
    seneca.add(
      { role: 'auth', cmd: 'auth-' + service },
      _login_service(service)
    )
    seneca.add(
      { role: 'auth', cmd: 'auth-' + service + '-callback' },
      _service_callback(service)
    )
    ...
    map['auth-' + service] = {
      ...
      POST: true,
      alias: '/' + service,
      responder: _blank_responder
    }
    map['auth-' + service + '-callback'] = {
      GET: true,
      POST: true,
      alias: '/' + service + '/callback'
    }
    ...
    seneca.add(
      { role: 'auth', trigger: 'service-login-' + service },
    ...
    function init_session(req, res, done) {
      req.seneca.act(
        ...
        function(err, result) {
          ...
          if (token) {
            seneca.act("role:'user',cmd:'auth'", { token: token }, function(
              err,
              out
            ) {
              if (err) return done(err)
          ...
          if (restrict && !(req.seneca && req.seneca.user)) {
            req.seneca.act(
              ...
              function(err, redirect) {
                ...
                if (redirect) {
                  return next({
                    http$: { status: 302, redirect: options.redirect.restrict }
                  })
                } else {
                  return next({
                    ok: false,
  ...
  // LOGIN START
  function afterlogin(err, next, req) {
    if (err) {
      return seneca.act(
        'role: auth, do: respond',
        { err: err, action: 'login', req: req },
        next
      )
    }
    ...
    if (req.user && req.user.ok) {
      ...
      req.seneca.act(
        ...
        {
          tokenkey: options.tokenkey,
          token: req.seneca.login.id || req.seneca.login.id
        },
        function(err) {
          return seneca.act(
            'role: auth, do: respond',
      ...
      seneca.act(
        'role: auth, do: respond',
        { err: err ? err.why : 'Unknown error', action: 'login', req: req },
        next
      )
    }
  }
      ...
      function(err, clienttoken) {
        ...
        req.seneca.act(
          ...
          function(err) {
            ...
            seneca.act("role:'user',cmd:'logout'", { token: token }, function(
              err
            ) {
              if (err) {
  ...
  function checkurl(match, done) {
    seneca.act("role:'auth',cmd: 'urlmatcher'", { spec: match }, function(
      err,
      checks
    ) {
      if (err) return done(err)
  ...
  if (!_.isFunction(options.restrict)) {
    seneca.act(
      "role:'auth', cmd: 'urlmatcher'",
      { spec: options.restrict },
      function(err, result) {
        if (!err) {
          checks = result
        }
      ...
      use: {
        ...
        map: {
          login: {
            ...
            GET: true,
            data: true,
            alias: options.urlpath.login
          },
          logout: {
            POST: true,
            GET: true,
            data: true,
            alias: options.urlpath.logout
          },
          register: {
            POST: authcontext,
            data: true,
            alias: options.urlpath.register
          },
          instance: { GET: authcontext, alias: options.urlpath.instance }, // this is deprecated - use user instead
          create_reset: {
            POST: authcontext,
            data: true,
            alias: options.urlpath.create_reset
          },
          load_reset: {
            POST: authcontext,
            data: true,
            alias: options.urlpath.load_reset
          },
          execute_reset: {
            POST: authcontext,
            data: true,
            alias: options.urlpath.execute_reset
          },
          confirm: {
            POST: authcontext,
            data: true,
            alias: options.urlpath.confirm
          },
          update_user: {
            POST: authcontext,
            data: true,
            alias: options.urlpath.update_user
          },
          user: { GET: authcontext, alias: options.urlpath.user },
          change_password: {
            POST: authcontext,
            data: true,
            alias: options.urlpath.change_password
          }
        }
      }
    })
