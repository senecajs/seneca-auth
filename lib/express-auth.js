...
module.exports = function (options) {
  ...
  function cmd_login (msg, respond) {
    ...
    seneca.act("role: 'auth', hook: 'map_fields'", {action: 'login', data: msg.data}, function (err, userData) {
      ...
      pp_auth.local(req, res, function (loginerr, out) {
        if (loginerr || !(out && out.ok)) {
          seneca.act('role: auth, do: respond', {err: (loginerr || (out && out.why)), action: 'login', req: req}, respond)
        }
  ...
  function trigger_service_login (msg, respond) {
    ...
    seneca.act('role: user, get: user', q, function (err, data) {
      ...
      var user = data.user
      if (!user) {
        seneca.act('role:user, restrict: register', user_data, function (err, res) {
          if (err) {
            return respond(null, {ok: false, why: err})
          }
          if (!res || !res.ok) {
            return respond(null, {ok: false, why: res.why || 'Register operation is restricted for this user'})
          }
        ...
        seneca.act('role:user, restrict: login', user_data, function (err, res) {
          if (err) {
            return respond(null, {ok: false, why: err})
          }
          if (!res || !res.ok) {
            return respond(null, {ok: false, why: res.why || 'Login operation is restricted for this user'})
          }
  ...
  function default_restrict_register(args, done){
    done(null, {
      ok: true,
      why: 'no-restrict'
    })
  ...
  function default_restrict_login(args, done){
    done(null, {
      ok: true,
      why: 'no-restrict'
    })
  ...
  seneca
    ...
    .add('role: auth, cmd: register_service', cmd_register_service)
    .add('role: auth, restrict: register', default_restrict_register)
    .add('role: auth, restrict: login', default_restrict_login)
