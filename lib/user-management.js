module.exports = function(options) {
      ...
      function(err, details) {
        ...
        if (err) {
          return seneca.act(
            ...
            { err: err, action: 'register', req: req },
            respond
          )
        }
        ...
        seneca.act("role:'user',cmd:'register'", details, function(err, out) {
          if (err || !out.ok) {
            return seneca.act(
              'role: auth, do: respond',
              {
                err: err || (out ? out.why : 'Internal server error'),
                action: 'register',
                req: req
              },
              respond
              ...
              if (req && req.seneca) {
                ...
                req.seneca.act(
                  "role: 'auth', set: 'token'",
                  {
                    ...
                    token: req.seneca.login.token || req.seneca.login.id
                  },
                  function(err) {
                    return seneca.act(
                      'role: auth, do: respond',
  ...
  function cmd_create_reset(msg, respond) {
    seneca.act(
      "role: 'auth', hook: 'map_fields'",
      { action: 'create_reset', data: msg.data },
  ...
  function cmd_load_reset(msg, respond) {
    ...
    seneca.act("role:'user',cmd:'load_reset'", { token: token }, function(
      err,
      out
    ) {
      if (err || !out.ok) {
  ...
  function cmd_execute_reset(msg, respond) {
    ...
    seneca.act(
      ...
      { token: token, password: password, repeat: repeat },
      respond
    )
  }
