...
ServiceRouter.prototype._authRequest = function(req, res, next) {
  ...
  self._passport.authenticate(serviceName, self._getService(req).conf(), func)(req, res, next)
}
...
ServiceRouter.prototype._buildDispatcher = function() {
  ...
  routes[self._options.prefix + self._templatize('service')] = function (req, res, next) {
  ...
  routes[self._options.prefix + self._templatize('service') + '/callback'] = function (req, res, next) {
    self._authRequest(req, res, function (err) {
      if (err) {
        next(err)
      } else {
