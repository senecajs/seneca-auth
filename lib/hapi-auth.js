...
module.exports = function(options) {
  ...
  function validateToken(msg, done) {
    ...
    req.seneca.act('role: user,cmd: auth', { token: clienttoken }, function(
      err,
      out
    ) {
      if (err) {
  ...
  function afterlogin(err, next, req, res) {
    if (err) {
      return req.seneca.act(
        ...
        { err: err, action: 'login', req: req, res: res },
        next
      )
    }
        ...
        req.seneca.act(
          ...
          function(err, out) {
            if (err || !(out && out.ok)) {
              ...
              req.seneca.act('role: auth, restrict: login', out, function(
                err,
                out
              ) {
                if (err || !(out && out.ok)) {
  ...
  function registerAuthService(args, done) {
    ...
    seneca.add(
      { role: 'auth', trigger: 'service-register-' + service },
      trigger_service_register
    )
    seneca.add(
  ...
  function trigger_service_login(args, respond) {
    ...
    req.seneca.act(
      'role: auth, prepare: ' + service + '_login_data',
      {
  ...
  function trigger_service_register(msg, respond) {
    ...
    seneca.act(
      "role: 'auth', identify: 'user'",
      { user: user_data, service: service },
      function(err, data) {
        ...
        var user = data.user
        if (!user) {
          seneca.act("role:'user',cmd:'register'", user_data, function(
            err,
            out
          ) {
            if (err) {
