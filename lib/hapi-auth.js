...
module.exports = function (options) {
  ...
  internals.map = {
    ...
    user: {GET: true, POST: true, alias: options.urlpath.user},
  ...
  internals.init_strategies = function () {
    if (!internals.options || !internals.options.strategies) {
      return
    }

    if (!_.isArray(internals.options.strategies)) {
      return
    }
    for (var i in internals.options.strategies) {
      var strategy = internals.options.strategies[i]

      if ('local' === strategy.provider) {
        internals.set_local_auth(strategy)
      }
      else {
        internals.set_auth_provider(strategy)
      }
    }


    // set cookie session strategy
    internals.seneca.act('role: web, get: server', function (err, data) {
      ...
      server = data.server
      server.auth.strategy('session', 'cookie', cookie_strategy)
    })

  }
  internals.set_auth_provider = function (strategy) {
  ...
  function auth_startware (msg, done) {
    ...
    req.seneca.act("role: 'auth', get: 'token'", {tokenkey: options.tokenkey}, function (err, clienttoken) {
      ...
      req.seneca.act('role: user,cmd: auth', {token: clienttoken}, done)
    })
  }
